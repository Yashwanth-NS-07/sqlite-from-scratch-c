...
void *get_page(Pager *pager, uint32_t page_num) {
    ...
    if(pager->pages[page_num] == NULL) {
        // cache miss, allocate mem
        void *page = malloc(PAGE_SIZE);
        uint32_t num_pages = pager->file_length / PAGE_SIZE;

        // check for partially filled page 
...
uint32_t *internal_node_child(void *node, uint32_t child_num) {
    ...
    if(child_num > num_keys) {
        printf("Tried to access the child_num %d > num_keys %d.\n", child_num, num_keys);
        exit(EXIT_FAILURE);
...
void initialize_internal_node(void *node) {
    ...
    *internal_node_num_keys(node) = 0;
    /*
    Necessary because the root page number is 0; but not initializing an internal
...
Cursor *leaf_node_find(Table *table, uint32_t page_num, uint32_t key) {
    ...
    while(min_index < max_index) {
        uint32_t index = (max_index + min_index) >> 1;
...
void create_new_root(Table *table, uint32_t right_child_page_num) {
    ...
    */
    ...
    // left child has the data copied from root
...
void internal_node_split_and_inset(Table *table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    after the old node's keys have been transfered over. We are not
    able to do this if the newly created node's parent is not a newly
    initialized root node, because in that case it's parent may have existing
    keys aside from our old node which we are splitting. If that is true, we
    need to find to place for our newly created node in its parent. and we
    ...
    */
    ...
    if(splitting_root) {
        ...
        /*
        If we are splitting the root, we need to udpate old_node to point
    ...
    /*
    First put right child into new node and set right chlid of old node to invalid page number
    */
...
void internal_node_insert(Table *table, uint32_t parent_page_num, uint32_t child_page_num) {
    ...
    */
    ...
    if(original_num_keys >= INTERNAL_NODE_MAX_CELLS) {
        internal_node_split_and_inset(table, parent_page_num, child_page_num);
        return;
    }
    
    uint32_t right_child_page_num = *internal_node_right_child(parent);
    /*
    An internal node with a right child a INVALID_PAGE_NUM is empty
    */
...
Pager *pager_open(const char *filename) {
    ...
    for(int i = 0; i < (sizeof(pager->pages))/sizeof(pager->pages[0]); i++) {
        pager->pages[i] = NULL;
    }
    return pager;
}
...
int main(int argc, char *argv[]) {
    if(argc < 2) {
        printf("Must supply the database filename.\n");
        exit(EXIT_FAILURE);
    }
